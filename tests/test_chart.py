...
def test_chart_metadata_tracks_runtime_independently() -> None:
    ...
    assert metadata["appVersion"] == "0.33.0"
    assert metadata["annotations"]["artifacthub.io/operator"] == "false"


def test_kind_validates_runtime_v033_lifecycle_contract() -> None:
    """Compatibility CI should bind the release and lifecycle documentation."""
    ...
    assert "ref: v0.33.0" in workflow
    ...
    assert "blob/v0.33.0/docs/ERRORS.md" in root_readme
    assert "blob/v0.33.0/docs/ERRORS.md" in chart_readme
    assert "blob/v0.33.0/docs/LIFECYCLE.md" in root_readme
    assert "blob/v0.33.0/docs/LIFECYCLE.md" in chart_readme
    assert "blob/v0.33.0/docs/HEALTH.md" in root_readme
    assert "blob/v0.33.0/docs/HEALTH.md" in chart_readme
    assert "blob/v0.33.0/docs/DASHBOARDS.md" in root_readme
    assert "blob/v0.33.0/docs/DASHBOARDS.md" in chart_readme
    assert "blob/v0.33.0/docs/ALERTING.md" in root_readme
    assert "blob/v0.33.0/docs/ALERTING.md" in chart_readme
    assert "blob/v0.33.0/deploy/observability/prometheus/rules/" in root_readme
    assert "blob/v0.33.0/deploy/observability/prometheus/rules/" in chart_readme
...
def test_default_render_preserves_production_runtime_contract() -> None:
    ...
    assert container["image"] == "ghcr.io/trussiumhq/trussium:0.33.0"
